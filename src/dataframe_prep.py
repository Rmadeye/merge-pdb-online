...
class data_prep:
    ...
    def read_and_correct(self, basename: str, flexname: str) -> str:
        ...
        rigid.to_pdb(records=['ATOM'], path = workdir+'\\rigid.tmp.pdb',
                     gz=False,
                     append_newline=True)
        flex.to_pdb(records=['ATOM'], path = workdir+'\\flex.tmp.pdb',
                     gz=False,
                     append_newline=True)

        return patcher.patch(workdir + '\\rigid.tmp.pdb', workdir+'\\flex.tmp.pdb', 'result.pdb')